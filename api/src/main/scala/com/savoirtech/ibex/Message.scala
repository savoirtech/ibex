...
case class Message(body: AnyRef, headers: Map[String, AnyRef] = Map()) {
   ...
   */
  def withHeaders(newHeaders: Map[String, AnyRef]): Message = Message(body, newHeaders)
   ...
   */
  def withHeaders(newHeaders: (String, AnyRef)*): Message = Message(body, headers ++ newHeaders)
   ...
   */
  def withHeader(key: String, value: AnyRef): Message = Message(body, headers + (key -> value))
}
